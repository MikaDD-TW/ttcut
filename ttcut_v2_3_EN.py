...
Changelog
    V2.3-EN 2026-09-13
        - English interface build of V2.3. Identical scoring, cutting, stats and
          rendering behaviour; only user-facing text and comments differ.
    V2.3 2026-09-13
        - New "stats board": tick it and the last frame is frozen at the end of
          the film with a centred summary board drawn over it
        - Top half is the per-game score table (name -> games won -> each game),
          with the winning number in the accent colour
        - Bottom half splits left/right and lists points won, service win rate
          and longest streak for each player
        - The longest streak carries across games: two 11:0 games in a row is 22
        - Hold duration is adjustable (1s by default); the audio is padded with
          silence over the freeze so picture and sound stay the same length
        - Stored in the tags JSON under stats; the CLI adds --stats / --no-stats
          / --stats-hold
        - With the box unticked, the generated .ass and filter graph are
          byte-identical to V2.2
        - Also fixed: Homebrew has no ffmpeg formula, so the instructions
          now say brew install ffmpeg
    V2.2-EN 2026-08-30
...
Requirements:
    ...
    Mac    : brew install ffmpeg     (the subtitles filter needs libass)
...
C_DIM      = "&HCEB28F&"      # secondary text: muted blue-grey

# ─────────────────────────────────────────── Stats board (also authored at 1920x1080)

SB_PAD         = 54           # board inner padding
SB_RULE        = 2            # divider thickness
SB_HEAD_H      = 42           # GAMES / PTS header of the score table
SB_ROW_H       = 86           # one row per player in the score table
SB_GAP         = 36           # breathing room between the table and the metrics
SB_TITLE_H     = 66           # metrics section heading
SB_NAMES_H     = 54           # the left/right player-name row above the metrics
SB_STAT_ROW_H  = 78           # one row per metric
SB_NAME_W      = 420          # width reserved for names in the score table
SB_GAMES_W     = 124          # games-won column (before the per-game scores, as on TV)
SB_COL_W       = 74           # per-game column; shrinks automatically for long matches
SB_MIN_W       = 1180         # minimum board width
A_BOARD        = 0x0C         # more solid than the corner scoreboard, for legibility

DEFAULT_STATS_HOLD = 1.0      # seconds the board is held at the end

STATS_TITLE  = "MATCH STATS"
STATS_GAMES  = "GAMES"
STATS_PTS    = "PTS"
STATS_LABELS = ("Total points", "Service win rate", "Longest point streak")
...
def ass_text(s):
    """ASS has no escape character and treats braces and backslashes as tag syntax, so swap them out of player names."""
...
def fold_full(events, fmt, start, first_server=0):
    ...
    snaps = []
    plays = []                             # per-point detail, used by the end-of-film stats board
    ...
    for e in events:
        ...
        if pending:
            ...
            a, b = init_pts(gi)
            pending = False
        srv = server                       # who served this point (rotation is updated later in this loop)
        if e["winner"] == "A":
            a += 1
        else:
            b += 1
...
def _better(x, y):
    """Return the side (0/1) with the larger value, or None if level."""
    return 0 if x > y else 1 if y > x else None


def match_stats(sc, start, players):
    """Turn the fold result into the numbers the stats board needs.

    games     score and winner of each game; a manual game change with no points
              leaves a None placeholder
    points    rallies actually won (handicap starting points do not count)
    serveRate points won on own serve / points served
    streak    longest run of consecutive points, carried across games (two 11:0
              games in a row is 22)
    """
    plays = sc["plays"]
...
def stats_lines(stats, k, width, height, c_accent):
    """End-of-film stats board; returns a list of (layer, style, text).

    The top half is the per-game score table: name, games won, then each game's
    score, with the winning number bright and the losing one dimmed. The bottom
    half is a side-by-side comparison, A's figure on the left and B's on the
    right with the metric name between them, the better side in the accent."""
    ...
    pad = S(SB_PAD)
    lx, rx = x0 + pad, x0 + pw - pad              # content margins
    ...
    # Bottom half: side-by-side metrics
...
"""
    ...
    # Stats board: the corner scoreboard ends at total, then the board has the screen
    if stats and hold > 0:
...
def filter_script(keeps, ass_name, fps, tonemap=False, hold=0.0):
    ...
    if hold > 0:                        # Freeze the last frame and pad the audio to match, or the streams drift apart
...
def plan(doc, opt):
    ...
    # Stats board: command-line settings win, then the tags JSON
    sb = doc.get("stats", {}) or {}
...
def summary_lines(plan_d, opt, video):
    ...
    if plan_d.get("hold"):
        out.append(f"Stats     board held for {plan_d['hold']:.1f}s at the end")
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .statbox{border-top:1px solid var(--line-soft);padding:11px 14px;
    font-size:12px;color:var(--ink-dim)}   /* live preview of the stats board */
...
<div class="shell">
  ...
  <div class="rail">
    ...
    <div class="statbox" id="statbox" hidden>
      <div class="hd"><span>match summary</span><span id="sbResult">0–0</span></div>
    ...
    <div class="render">
      ...
      <div class="line">
        <label class="ctl"><input type="checkbox" id="stats">Stats board</label>
        <div class="ctl" style="margin-left:auto">hold<input type="number"
...
(() => {
  ...
  /* ───────────────────────── Stats board: see the numbers in the interface without rendering first */
  ...
  function paintStats(st) {
    ...
    $('statbox').hidden = !on || !s;
    if (!on || !s) return;
    const nm = names(), L = {"games": "Games", "l1": "Points won", "l2": "Service win", "l3": "Longest streak", "dash": "–", "none": "—"};
...
def main():
    ...
    p.add_argument("--stats", dest="stats", action="store_true", default=None,
                   help="freeze the last frame and draw the stats board over it")
    p.add_argument("--no-stats", dest="stats", action="store_false",
                   help="no stats board, overriding the tags JSON")
    p.add_argument("--stats-hold", type=float, default=None,
                   help=f"seconds to hold the stats board, default {DEFAULT_STATS_HOLD}")
